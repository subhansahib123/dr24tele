                        ...
                        <div class="row">
                            <div class="col-md-12 col-xl-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form class="form-horizontal" action="{{ route('image_categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                                            @method('patch')
                                            @csrf
                                            <div class=" row mb-4">
                                                ...
                                                <div class="col-md-9">
                                                    <select class="form-control"  name="parent_id" >
                                                        ...
                                                        @foreach ($categories as $category_parent)
                                                            <option {{$category->parent_id==$category_parent->id?"selected":""}}  value="{{$category_parent->id}}">{{$category_parent->title}}</option>

                                                            @foreach($category_parent->subCategories as $option)
                                                                <option   {{$category->parent_id==$option->id?"selected":""}} value="{{$option->id}}">--{{$option->title}}</option>
                                                                @foreach($option->subCategories as $option_sub)

                                                                    @if (count($option_sub->subCategories)>0)
                                                                        @foreach($option->subCategories as $option_sub)
                                                                        <option  {{$category->parent_id==$option_sub->id?"selected":""}}  value="{{$option_sub->id}}">------{{$option_sub->title}}</option>
                                                                        @endforeach
                                                                    {{-- @else
                                                                    <option value="{{$option_sub->id}}">----{{$option_sub->title}}</option> --}}
                                                                    @endif
                                                                @endforeach
                                                            @endforeach
                                                        @endforeach
                                            ...
                                            <div class=" row mb-4">
                                                ...
                                                <img src="{{asset($category->image)}}" class="w-20" />
                                                 @if ($errors->has('image'))
